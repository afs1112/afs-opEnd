...
function testSensorTypeIdentification() {
  ...
  sensors.forEach(sensor => {
    const isLaser = sensor.type.toLowerCase().includes('laser') || 
                   sensor.name.toLowerCase().includes('laser');
    const canLase = isLaser && (sensor.type.includes('Designator') || sensor.type.includes('Rangefinder'));
  ...
  sensorCommands.forEach((cmd, index) => {
    ...
    try {
      ...
      if (cmd.requiresParams) {
        console.log(`      - 方位角: ${cmd.params.azSlew}°, 俯仰角: ${cmd.params.elSlew}°`);
      }
...
function testCommandEnableLogic() {
  ...
  const testScenarios = [
    ...
    {
      name: '选择平台和光电传感器',
      state: { platform: 'UAV-001', sensor: 'EO-Pod-1', sensorType: 'Electro-Optical', weapon: '', target: '' },
      expected: {
        'Uav_Sensor_On': true,
    ...
    {
      name: '选择平台和激光传感器',
      state: { platform: 'UAV-001', sensor: 'Laser-Pod-1', sensorType: 'Laser-Designator', weapon: '', target: '' },
      expected: {
        'Uav_Sensor_On': true,
        'Uav_Sensor_Turn': true,
    ...
    {
      name: '完整选择',
      state: { platform: 'ARTY-001', sensor: 'Fire-Control-Radar', sensorType: 'Fire-Control', weapon: 'Howitzer-1', target: 'Target-003' },
      expected: {
        'Uav_Sensor_On': true,
        'Uav_Sensor_Turn': true,
        'Uav_LazerPod_Lasing': false,  // 非激光传感器
  ...
  testScenarios.forEach(scenario => {
    ...
    // 模拟命令启用逻辑
    const isLaserSensor = scenario.state.sensorType?.toLowerCase().includes('laser') || 
                         scenario.state.sensor?.toLowerCase().includes('laser') || false;
    
    const actualEnabled = {
      'Uav_Sensor_On': !!(scenario.state.platform && scenario.state.sensor),
      'Uav_Sensor_Turn': !!(scenario.state.platform && scenario.state.sensor),
      'Uav_LazerPod_Lasing': !!(scenario.state.platform && scenario.state.sensor && isLaserSensor),
      'Arty_Fire': !!(scenario.state.platform && scenario.state.weapon && scenario.state.target)
    };
    ...
    Object.entries(scenario.expected).forEach(([command, expected]) => {
      ...
      console.log(`   - ${command}: ${match ? '✅' : '❌'} (期望: ${expected}, 实际: ${actual})`);
...
function testUIComponentLogic() {
  ...
  mockPlatforms[0].sensors.forEach(sensor => {
    const isLaser = sensor.type.toLowerCase().includes('laser') || 
                   sensor.name.toLowerCase().includes('laser');
    const displayClass = isLaser ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600';
  ...
  // 测试按钮启用状态
  const testStates = [
    { platform: '', sensor: '', expected: { sensorOn: false, laserLasing: false } },
    { platform: 'UAV-001', sensor: '', expected: { sensorOn: false, laserLasing: false } },
    { platform: 'UAV-001', sensor: 'EO-Pod-1', expected: { sensorOn: true, laserLasing: false } },
    { platform: 'UAV-001', sensor: 'Laser-Pod-1', expected: { sensorOn: true, laserLasing: true } }
  ];
  
  console.log('\\n   📋 按钮启用状态测试:');
  testStates.forEach((state, index) => {
    const selectedSensor = mockPlatforms[0].sensors.find(s => s.name === state.sensor);
    const isLaser = selectedSensor?.type.toLowerCase().includes('laser') || false;
    ...
    const match = actualEnabled.sensorOn === state.expected.sensorOn && 
    ...
    console.log(`   - 传感器开启按钮: ${actualEnabled.sensorOn ? '启用' : '禁用'}`);
    console.log(`   - 激光照射按钮: ${actualEnabled.laserLasing ? '启用' : '禁用'}`);
