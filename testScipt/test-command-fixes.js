  ...
  try {
    ...
    if (
      ...
      platforms.forEach((platform, index) => {
        ...
        console.log(
          `      - 通信: ${platform.comms.map((c) => c.name).join(", ")}`
        );
        console.log(
          `      - 传感器: ${platform.sensors.map((s) => s.name).join(", ")}`
        );
        console.log(
          `      - 武器: ${platform.weapons.map((w) => w.name).join(", ")}`
        );
        console.log(
          `      - 目标: ${platform.tracks.map((t) => t.targetName).join(", ")}`
        );
      });
...
function testMulticastPacketHandling() {
        ...
        platform: [
          {
            ...
            weapons: [
              { base: { name: "TestWeapon", type: "Missile" }, quantity: 2 },
            ],
            tracks: [
              {
  ...
  console.log("   📋 模拟数据包处理:");
  console.log(
    `   - 时间戳: ${new Date(mockPacket.timestamp).toLocaleString()}`
  );
...
function testUIStateUpdate() {
  ...
  if (currentPlatform) {
    ...
    console.log(
      `      - 通信: ${currentPlatform.comms.map((c) => c.name).join(", ")}`
    );
    console.log(
      `      - 传感器: ${currentPlatform.sensors.map((s) => s.name).join(", ")}`
    );
    console.log(
      `      - 武器: ${currentPlatform.weapons.map((w) => w.name).join(", ")}`
    );
    console.log(
      `      - 目标: ${currentPlatform.tracks
        .map((t) => t.targetName)
        .join(", ")}`
    );
  }
