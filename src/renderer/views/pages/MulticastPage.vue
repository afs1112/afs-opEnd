<template>
  <div class="flex flex-col h-full p-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-8 gap-4">
        <div class="text-center">
          <div
            ...
            :class="isListening ? 'text-green-600' : 'text-red-600'"
          >
            {{ isListening ? "监听中" : "已停止" }}
          </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6 flex-1 overflow-hidden">
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="flex gap-2">
          <el-dropdown @command="handleBatchCopyCommand">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item command="all-parsed"
                  >复制所有解析数据</el-dropdown-item
                >
                <el-dropdown-item command="all-hex"
                  >复制所有十六进制</el-dropdown-item
                >
                <el-dropdown-item command="all-full"
                  >复制所有完整信息</el-dropdown-item
                >
                <el-dropdown-item command="summary"
                  >复制数据包摘要</el-dropdown-item
                >
              </el-dropdown-menu>
            </template>
          </el-dropdown>
      ...
      <!-- 平台状态包汇聚显示 -->
      <div
        v-if="platformStatusPackets.length > 0"
        class="bg-orange-50 rounded-lg p-4 mb-4 border border-orange-200"
      >
        <div class="flex justify-between items-center mb-2">
          <div class="flex items-center gap-2">
            <div class="text-orange-700 font-semibold">📊 平台状态汇聚</div>
            <el-tag size="small" type="warning"
              >{{ platformStatusPackets.length }} 个</el-tag
            >
            <el-tag
              size="small"
              type="success"
            ...
            >
              ...
              {{
                formatTime(
                  platformStatusPackets[platformStatusPackets.length - 1]
                    .timestamp
                )
              }}
            </el-tag>
          ...
          <div class="flex gap-2">
            <el-button size="small" @click="copyPlatformStatusSummary"
              >复制状态摘要</el-button
            >
            <el-button size="small" @click="clearPlatformStatus"
              >清空状态</el-button
            >
            <el-button
              size="small"
              @click="showPlatformStatus = !showPlatformStatus"
            >
        ...
        <div v-if="showPlatformStatus" class="max-h-40 overflow-y-auto">
          ...
          <div class="space-y-1">
            <div
              v-for="(status, index) in platformStatusPackets
                .slice(-10)
                .reverse()"
              :key="index"
            ...
            >
              <div class="flex gap-4">
                ...
                <span v-if="status.parsedPacket">
                  平台数:
                  {{ status.parsedPacket.parsedData?.platform?.length || 0 }}
                </span>
              </div>
              <div class="flex gap-1">
                <el-button
                  ...
                  @click="
                    showPacketDetail(
                      status,
                      platformStatusPackets.indexOf(status)
                    )
                  "
                >
                  详情
                ...
                <el-button
                  ...
                  @click="
                    copyToClipboard(
                      JSON.stringify(
                        status.parsedPacket?.parsedData || {},
                        null,
                        2
                      ),
                      '平台状态数据'
      ...
      <!-- 平台命令包汇聚显示 -->
      <div
        v-if="platformCmdPackets.length > 0"
        class="bg-cyan-50 rounded-lg p-4 mb-4 border border-cyan-200"
      >
        <div class="flex justify-between items-center mb-2">
          <div class="flex items-center gap-2">
            <div class="text-cyan-700 font-semibold">🎮 平台命令汇聚</div>
            <el-tag size="small" type="info"
              >{{ platformCmdPackets.length }} 个</el-tag
            >
            <el-tag
              size="small"
              type="success"
            ...
            >
              ...
              {{
                formatTime(
                  platformCmdPackets[platformCmdPackets.length - 1].timestamp
                )
              }}
            </el-tag>
          </div>
          <div class="flex gap-2">
            <el-button size="small" @click="copyPlatformCmdSummary"
              >复制命令摘要</el-button
            >
            <el-button size="small" @click="clearPlatformCmd"
              >清空命令</el-button
            >
        ...
        <div v-if="showPlatformCmd" class="max-h-40 overflow-y-auto">
          ...
          <div class="space-y-1">
            <div
              v-for="(cmd, index) in platformCmdPackets.slice(-10).reverse()"
              :key="index"
              class="bg-white rounded p-2 text-xs flex justify-between items-center"
            >
              <div class="flex gap-4">
                ...
                <span v-if="cmd.parsedPacket">
                  命令:
                  {{ getCommandName(cmd.parsedPacket.parsedData?.command)
                  }}{{
                    getCoordinationCommandDetails(cmd.parsedPacket.parsedData)
                  }}
                </span>
              </div>
              <div class="flex gap-1">
                <el-button
                  ...
                  plain
                  @click="
                    showPacketDetail(cmd, platformCmdPackets.indexOf(cmd))
                  "
                >
                  详情
                </el-button>
      ...
      <!-- 导航数据汇聚显示 -->
      <div
        v-if="navDataPackets.length > 0"
        class="bg-indigo-50 rounded-lg p-4 mb-4 border border-indigo-200"
      >
        <div class="flex justify-between items-center mb-2">
          <div class="flex items-center gap-2">
            <div class="text-indigo-700 font-semibold">🛩️ 导航数据汇聚</div>
            <el-tag size="small" type="primary"
              >{{ navDataPackets.length }} 个</el-tag
            >
            <el-tag
              size="small"
              type="success"
            ...
            >
              最新:
              {{
                formatTime(navDataPackets[navDataPackets.length - 1].timestamp)
              }}
            </el-tag>
          </div>
          <div class="flex gap-2">
            <el-button size="small" @click="copyNavDataSummary"
              >复制导航摘要</el-button
            >
            <el-button size="small" @click="clearNavData"
              >清空导航数据</el-button
            >
        ...
        <div v-if="showNavData" class="max-h-60 overflow-y-auto">
          ...
          <div class="space-y-1">
            <div
              v-for="(navData, index) in navDataPackets.slice(-20).reverse()"
              :key="index"
              class="bg-white rounded p-2 text-xs flex justify-between items-center"
            >
              <div class="flex gap-4">
                ...
                <span v-if="navData.parsedPacket">
                  <el-tag
                    size="small"
                    :type="getNavDataTypeTag(navData.parsedPacket.packageType)"
                  >
                ...
                <span
                  v-if="
                    navData.parsedPacket &&
                    navData.parsedPacket.packageType === 0x1
                  "
                >
                ...
                <span
                  v-if="
                    navData.parsedPacket &&
                    navData.parsedPacket.packageType === 0x20
                  "
                >
                  UavID: {{ navData.parsedPacket.parsedData?.uavID || "N/A" }},
                  航点:
                  {{ navData.parsedPacket.parsedData?.wayPointSize || 0 }}个
                </span>
              </div>
              <div class="flex gap-1">
                <el-button
                  ...
                  plain
                  @click="
                    showPacketDetail(navData, navDataPackets.indexOf(navData))
                  "
                >
                  详情
                </el-button>
                <el-button
                  ...
                  @click="
                    copyToClipboard(
                      JSON.stringify(
                        navData.parsedPacket?.parsedData || {},
                        null,
                        2
                      ),
                      '导航数据'
  ...
  <el-dialog
    ...
    :title="`数据包详情 #${selectedPacketIndex + 1}`"
    width="80%"
  ...
  >
    <div v-if="selectedPacket" class="space-y-4">
      ...
      <div class="bg-gray-50 rounded-lg p-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div class="bg-white rounded p-3">
            ...
            <div class="font-mono">
              {{ formatTime(selectedPacket.timestamp) }}
            </div>
          </div>
          <div class="bg-white rounded p-3">
            <div class="text-gray-500 text-xs">源地址</div>
            <div class="font-mono">
              {{ extractSourceIP(selectedPacket.source) }}
            </div>
          </div>
          <div class="bg-white rounded p-3">
            <div class="text-gray-500 text-xs">源端口</div>
            <div class="font-mono">
              {{ extractSourcePort(selectedPacket.source) }}
            </div>
          </div>
          <div class="bg-white rounded p-3">
      ...
      <!-- 解析信息 -->
      <div
        v-if="selectedPacket.parsedPacket"
      ...
      >
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm mb-4">
          <div class="bg-white rounded p-3">
            ...
            <div class="font-mono">
              {{ selectedPacket.parsedPacket.packageTypeName }}
            </div>
          </div>
          <div class="bg-white rounded p-3">
            ...
            <div class="font-mono">
              0x{{
                selectedPacket.parsedPacket.packageType
                  .toString(16)
                  .padStart(2, "0")
              }}
            </div>
          ...
          <div class="bg-white rounded p-3">
            ...
            <div class="font-mono">
              0x{{
                selectedPacket.parsedPacket.protocolID
                  .toString(16)
                  .padStart(2, "0")
              }}
            </div>
        ...
        <div class="bg-white rounded-lg p-4">
          <div class="flex justify-between items-center mb-2">
            ...
            <el-button
              ...
              @click="
                copyToClipboard(
                  JSON.stringify(
                    selectedPacket.parsedPacket.parsedData,
                    null,
                    2
                  ),
                  '解析数据'
          ...
          <pre
            class="bg-gray-100 rounded p-3 text-xs overflow-auto max-h-60 font-mono"
            >{{
              JSON.stringify(selectedPacket.parsedPacket.parsedData, null, 2)
            }}</pre
          >
        </div>
      </div>
        ...
        <div class="bg-white rounded-lg p-4 mb-4">
          <div class="flex justify-between items-center mb-2">
            ...
            <el-button
              ...
              type="primary"
              @click="
                copyToClipboard(toHex(selectedPacket.data), '十六进制数据')
              "
            >
              复制十六进制
            </el-button>
          ...
          <pre
            class="bg-gray-100 rounded p-3 text-xs overflow-auto max-h-40 font-mono break-all"
        ...
        <div class="bg-white rounded-lg p-4">
          <div class="flex justify-between items-center mb-2">
            ...
            <el-button
              ...
              @click="
                copyToClipboard(
                  getBase64Data(selectedPacket.data),
                  'Base64数据'
                )
              "
            >
    ...
    <template #footer>
      <div class="flex justify-between">
        <div class="flex gap-2">
          <el-button
            size="small"
            :disabled="selectedPacketIndex <= 0"
          ...
          <el-button
            size="small"
            :disabled="selectedPacketIndex >= displayPackets.length - 1"
...
const displayPackets = computed(() => {
  ...
  return packets.value.filter(
    (p) => !excludedTypes.includes(p.parsedPacket?.packageType)
  );
});
...
const showPacketDetail = (packet: MulticastPacket, index: number) => {
  ...
  const allPacketsIndex = packets.value.findIndex(
    (p) =>
  ...
  const fullInfo = {
    ...
    解析信息: selectedPacket.value.parsedPacket
      ? {
          ...
          类型码: `0x${selectedPacket.value.parsedPacket.packageType
            .toString(16)
            .padStart(2, "0")}`,
          协议ID: `0x${selectedPacket.value.parsedPacket.protocolID
            .toString(16)
            .padStart(2, "0")}`,
  ...
  switch (command) {
    ...
    case "parsed":
      ...
      if (packet.parsedPacket) {
        copyToClipboard(
          JSON.stringify(packet.parsedPacket.parsedData, null, 2),
          "解析数据"
        );
      }
      break;
      ...
      const fullInfo = {
        ...
        解析信息: packet.parsedPacket
          ? {
              ...
              类型码: `0x${packet.parsedPacket.packageType
                .toString(16)
                .padStart(2, "0")}`,
              协议ID: `0x${packet.parsedPacket.protocolID
                .toString(16)
                .padStart(2, "0")}`,
  ...
  switch (command) {
    case "all-parsed":
      ...
      copyToClipboard(
        ...
        `${parsedData.length}个解析数据包`
      );
      break;

    case "all-hex":
      // 复制所有十六进制数据（从新到旧排序）
      const hexData = packets.value
        .slice()
        .reverse()
        .map((p, index) => ({
          序号: index + 1,
      ...
      copyToClipboard(
        JSON.stringify(hexData, null, 2),
        `${hexData.length}个数据包的十六进制数据`
      );
      break;

    case "all-full":
      // 复制所有完整信息（从新到旧排序）
      const allFullData = packets.value
        .slice()
        .reverse()
        .map((p, index) => ({
          序号: index + 1,
          ...
          解析信息: p.parsedPacket
            ? {
                包类型: p.parsedPacket.packageTypeName,
                类型码: `0x${p.parsedPacket.packageType
                  .toString(16)
                  .padStart(2, "0")}`,
                协议ID: `0x${p.parsedPacket.protocolID
                  .toString(16)
                  .padStart(2, "0")}`,
      ...
      copyToClipboard(
        JSON.stringify(allFullData, null, 2),
        `${allFullData.length}个完整数据包信息`
      );
      break;
      ...
      const summary = {
        ...
        最新数据包:
          packets.value.length > 0
            ? {
                时间: formatTime(
                  packets.value[packets.value.length - 1].timestamp
                ),
...
const getPlatformStatusDuration = () => {
  ...
  const last =
    platformStatusPackets.value[platformStatusPackets.value.length - 1]
      .timestamp;
...
const copyPlatformStatusSummary = () => {
  const summary = {
    平台状态统计: {
      ...
      来源列表: [
        ...new Set(
          platformStatusPackets.value.map((p) => extractSourceIP(p.source))
        ),
      ],
    },
    最近状态: platformStatusPackets.value
      .slice(-5)
      .reverse()
      .map((p) => ({
        时间: formatTime(p.timestamp),
...
const getPlatformCmdDuration = () => {
  ...
  const last =
    platformCmdPackets.value[platformCmdPackets.value.length - 1].timestamp;
  const duration = Math.floor((last - first) / 1000);
  return duration > 60
    ? `${Math.floor(duration / 60)}分${duration % 60}秒`
    : `${duration}秒`;
...
// 获取协同命令的详细信息
const getCoordinationCommandDetails = (parsedData: any) => {
...
const copyPlatformCmdSummary = () => {
  const summary = {
    平台命令统计: {
      ...
      来源列表: [
        ...new Set(
          platformCmdPackets.value.map((p) => extractSourceIP(p.source))
        ),
      ],
    },
    最近命令: platformCmdPackets.value
      .slice(-5)
      .reverse()
      .map((p) => ({
        时间: formatTime(p.timestamp),
...
const getUavStatusCount = () => {
  return navDataPackets.value.filter((p) => p.parsedPacket?.packageType === 0x1)
    .length;
};

const getRouteUploadCount = () => {
  return navDataPackets.value.filter(
    (p) => p.parsedPacket?.packageType === 0x20
  ).length;
};
...
const copyNavDataSummary = () => {
  const summary = {
    导航数据统计: {
      ...
      来源列表: [
        ...new Set(navDataPackets.value.map((p) => extractSourceIP(p.source))),
      ],
    },
    最近数据: navDataPackets.value
      .slice(-10)
      .reverse()
      .map((p) => ({
        时间: formatTime(p.timestamp),
...
const exportPackets = async () => {
  ...
  try {
    const filePath = await window.electronAPI.export.showSaveDialog({
      title: "导出组播数据",
      defaultFileName: `multicast_packets_${new Date()
        .toISOString()
        .slice(0, 19)
        .replace(/:/g, "-")}.json`,
      filters: [
